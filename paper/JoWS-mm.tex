...
The VisText RDF schema is shown in Table~\ref{tab:vistext_schema}. The schema is intentionally small so that evaluation focuses on whether the model recovers the chart semantics and data values, rather than on a large external ontology. Not every original VisText item yields a reliable RDF reference graph. Some original labels contain damaged categories, inconsistent tabular annotations, or insufficient information to recover the plotted data. Such items are excluded from labelled evaluation, while the remaining items are released as RDF/Turtle references.
...
\toprule
\textbf{Component} & \textbf{Vocabulary} & \textbf{Target information} \\
\midrule
Chart type & \texttt{:BarChart}, \texttt{:LineChart}, \texttt{:AreaChart} & Visual chart class \\
Chart resource & \texttt{:Chart} with \texttt{:title}, \texttt{:xAxis}, \texttt{:yAxis} & Title and axis links \\
Axis resources & \texttt{:XAxis}, \texttt{:YAxis}, \texttt{:Axis} & Visible axis titles \\
Data resources & \texttt{:DataPoint} & Individual observations \\
Data predicates & \texttt{:xValue}, \texttt{:yValue}, \texttt{:belongsTo} & Datapoint coordinates and chart membership \\
\bottomrule
\end{tabularx}
\caption{VisText chart-to-RDF target schema. The namespace is \texttt{http://example.org/vistext\#}.}
\label{tab:vistext_schema}
\end{table}

\subsubsection{Diagram2Graph-derived Diagram-to-RDF Dataset}
Diagram2Graph provides diagram images and JSON annotations describing nodes, edges, labels, shapes, and relationship types \citep{diagram2graph}. We use the dataset as a diagram-to-RDF benchmark by expressing the same graph information in Turtle. The target output contains one resource for each visible diagram node and one resource for each directed edge. Node resources encode the visible node label, node type, and shape. Edge resources encode source node, target node, edge style, relationship type, and optional visible edge label. This representation preserves the diagram topology while using a compact RDF vocabulary suitable for direct graph matching.
...
\textbf{Component} & \textbf{Vocabulary} & \textbf{Target information} \\
\midrule
Node classes & \texttt{:Node}, \texttt{:Start}, \texttt{:Process}, \texttt{:Decision}, \texttt{:Delay}, \texttt{:Terminator} & Diagram node type \\
