...
The input image will contain exactly one chart, and it will be one of:
- bar chart
- line chart
- area chart

You must classify the chart as exactly one of:
- :BarChart
- :LineChart
- :AreaChart
...
- Do **not** infer the number of data points only from visible bends, corners, or local shape changes in the plotted line or area boundary.
- The plotted curve/area is continuous in appearance, but the underlying dataset is discrete.
...
### For categorical line/area charts
...
When the x-axis represents yearly observations in a **line chart** or **area chart**:
