...
from categorize_viz import (  # noqa: E402
    ALLOWED_CATEGORIES,
...
def chart_type_from_classification_manifest(image_path: Path, manifest_path: Path) -> str | None:
    ...
    for item in manifest.get("items", []) or []:
        ...
        chart_type = item.get("chart_type") or item.get("predicted_chart_type")
        if status == "classified" and isinstance(chart_type, str):
...
def build_prompt_package(
    ...
    if effective_context.classification_manifest_path is not None:
        chart_type = chart_type_from_classification_manifest(
            image_path,
    ...
    if chart_type is not None:
        ...
        raise ValueError(
            f"Missing collected chart type for {image_path.stem} in "
...
def run_classification_batch_submit(config: RuntimeConfig, selected_images: list[Path]) -> int:
    ...
    uploaded_request_file = upload_jsonl_file(
        client,
        request_jsonl_path,
        display_name=f"{config.batch_display_name or 'vistext-dynamic-classify'}-requests-{timestamp}",
    )
    display_name = config.batch_display_name or f"vistext-dynamic-classify-{timestamp}"
    ...
    manifest = {
        ...
        "dataset": "vistext",
        "purpose": "dynamic_oneshot_chart_type_classification",
        "model": config.model,
...
def run_classification_batch_collect(config: RuntimeConfig) -> int:
    ...
    for record in result_records:
        ...
        if error:
            ...
            try:
                item["chart_type"] = normalize_chart_type(extract_batch_response_text(record))
